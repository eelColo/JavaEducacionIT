	...
	//Mamita se viene el rompecabezas
		...
		public void add() throws MyException {
			...
			try {
				...
				if(nombre.isEmpty()) throw new MyException("No puedes agregar un nombre en blanco!");
				
				
				
				System.out.println("Ingrese una descripcion del mismo: ");
				String des = entrada.nextLine();
				if(des.isEmpty()) throw new MyException("No puedes dejar la descripcion vacia!");
				
				
				System.out.println("Ingrese el precio: ");
				int precio = entrada.nextInt();
				if(precio <= 0) throw new MyException("El Precio no puede ser negativo!");
				
				
				System.out.println("Ingrese Stock actual: ");
				int stock = entrada.nextInt();
				if(stock <= 0) throw new MyException("El Stock no puede ser negativo!");
			...
			try {
				...
				if(operacion ==  1) {
					...
					try {
						...
						if(nuevoPrecio < 0 ) throw new MyException("No puedes asignar valor negativo a los productos!");
				...
				} else throw new MyException("Porfavor ingrese un valor numerico acorde a las opciones");
